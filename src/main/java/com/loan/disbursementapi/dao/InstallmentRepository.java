...
public interface InstallmentRepository extends JpaRepository<Installment, Integer> {
    ...
    List<Installment> findAllByCreditIdAndStatusIsNot(Integer creditId, InstallmentStatus status);

    @Query(value = "SELECT i FROM Installment i WHERE i.status != :status")
    List<Installment> findAllByStatusIsNot(InstallmentStatus status);
}
